...
9. **操作概要记入 log 文件**：每次执行的操作概要必须及时记入 `output/YYYY/MM/YYYY-MM-DD-log.md` 文件，包括操作类型、处理的文件、生成的文件、更新的索引等信息。
...
- **存储规则**：参考材料对话记录追加到 `output/YYYY/MM/<YYYY-MM-DD>-log.md`；处理后的内容保存到 `subjects/<学科>/YYYY/MM/`，文件名格式为 `<YYYY-MM-DD>-<descriptive-slug>.md`，在 frontmatter 中增加 `reference_type` 字段（`standard_answer`/`model_essay`/`classmate_essay`）
...
│   └── 生物/
│       └── _index.md
└── output/               # 学习产出（学情报告、练习文件、日志等，按年月归档）
    ├── YYYY/
    │   └── MM/
...
- **写入**：批改或归档错题时，写入错题文件并更新对应学科的 `_index.md`（追加薄弱点、重算正确率）；对话讲解结束后，追加会话摘要到 `output/YYYY/MM/<YYYY-MM-DD>-log.md` 并更新 `_index.md` 的最近错题记录。
