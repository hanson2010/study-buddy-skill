...
本助手提供 4 个子命令，用于启动不同的工作流程：

| 子命令 | 功能 | 适用场景 |
|--------|------|----------|
| `learn` | 知识点学习与巩固 | 学习知识点、复习薄弱点、古诗文学习 |
| `eval` | 作业批改与错题归档 | 上传作业/试卷照片、请求批改错题 |
| `feedback` | 成绩分析报告 | 上传成绩分析报告、单科卷面分析报告 |
| `report` | 学情追踪 | 生成学习周报/月报、分析学习趋势 |

**使用方式**：
- **命令形式**：直接输入子命令，如 `learn`、`eval`、`feedback`、`report`
...
```
~/.studybuddy/
├── profile.md                 # 学习背景档案（姓名、年级、文理科、教材版本）
...
| 序号 | 规则名称 | 说明 |
|------|----------|------|
| 1 | 难题破解五步法 | 错题辅导必须按「识别/批改 → 错因分析 → 可视化讲解 → 视频推荐 → 举一反三」顺序执行 |
...
- [feedback_workflow.md](skills/studybuddy/references/feedback_workflow.md) — 成绩分析反馈工作流
